...
export const agentChatRoutes: FastifyPluginAsync = async (app) => {
  ...
  // Agent catalog
  app.get("/agents", auth, async (_req, reply) => {
    return reply.send([
      { id: "assistant", name: "通用助手", desc: "日常问答与运维知识", color: "#6366f1" },
      { id: "inspection", name: "AI 巡检", desc: "健康巡检与风险报告", color: "#10b981" },
      { id: "diagnosis", name: "自主排查", desc: "多步取证定位根因", color: "#8b5cf6" },
      { id: "terminal", name: "AI 终端", desc: "自然语言生成并执行命令", color: "#f59e0b" },
      { id: "weekly", name: "AI 运维周报", desc: "自然语言汇总周期运维与价值", color: "#0ea5e9" },
    ]);
  });
};
