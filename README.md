<p align="center">
  <h1 align="center">🛰️ ProberX</h1>
  <p align="center">轻量级自托管服务器 & 网站监控平台</p>
  <p align="center">监控 · 运维 · 管理 — 一站式 ServerOps</p>
</p>

<p align="center">
  <a href="https://panel.yqone.cn"><img src="https://img.shields.io/badge/官网-panel.yqone.cn-2ea44f" alt="官网"></a>
  <a href="https://agent.yqone.cn"><img src="https://img.shields.io/badge/在线Demo-agent.yqone.cn-2ea44f" alt="在线Demo"></a>
  <a href="https://github.com/longdicode/ProberX/releases"><img src="https://img.shields.io/badge/下载-Releases-2ea44f" alt="下载"></a>
...
- **在线 / 离线双安装** — 面板内通过 SSH 远程一键安装 Agent（实时显示安装日志），或复制脚本离线安装
...
- **桌面客户端** — 基于 Electron 的 Windows / macOS / Linux 客户端，与 Web 端功能同步
...
| 入口 | 地址 | 说明 |
|------|------|------|
| 🏠 官网 | https://panel.yqone.cn | 产品介绍、功能展示、安装文档 |
| 🖥️ 在线 Demo | https://agent.yqone.cn | 可直接注册体验完整功能 |
| 📦 桌面端下载 | https://github.com/longdicode/ProberX/releases | Windows / macOS / Linux 安装包 |
...
- **公开状态页** — 自定义品牌、健康状态展示
- **PWA** — 可安装到桌面、离线可用
- **WebSSH 终端** — 浏览器直连服务器，Windows + Unix
- **API Key** — 工作空间级密钥，支持外部集成
- **Telegram Bot** — 独立服务，告警/查询/控制
...
┌──────────────────────────────────────────────────────────────┐
│                          用户层                                │
│    Web 管理台 (Next.js)  │  桌面客户端 (Electron)  │  Telegram  │
└──────────────────────────────┬───────────────────────────────┘
                               │ HTTPS / WSS
┌──────────────────────────────▼───────────────────────────────┐
│                       控制中心 (Dashboard)                     │
│   Fastify v5 │ BullMQ 队列 │ WebSocket 网关 │ 告警引擎         │
│   JWT/RBAC 多租户 │ 探测调度 │ 云备份 │ 状态页                │
└──────┬───────────────────────────────┬───────────────────────┘
       │ PostgreSQL 16 + TimescaleDB   │ Redis 7
       └───────────────┬───────────────┘
                       │ HTTPS / WSS（Agent 注册、心跳、推送、命令）
┌──────────────────────▼───────────────────────────────────────┐
│                      探针层 (Agent)                           │
...
> **第一步：获取 Token 和 Agent ID**
> 登录后台 →「服务器」→「创建服务器」，创建后会自动生成 `AGENT_TOKEN` 和 `AGENT_ID`（也可在服务器详情中重新生成 Token）。

#### 方式一：在线安装（推荐）

在后台「服务器」→「创建服务器」中选择**在线安装**，填写目标服务器的 SSH 连接信息，面板会自动通过 SSH 远程安装 Agent 并自动填入 Token / Agent ID，同时**实时显示安装命令执行进度**。
...
脚本会自动下载 Agent 二进制、配置 systemd 服务并启动。
...
> 自托管部署时，将命令中的 `http://agent.yqone.cn:4000` 替换为你自己的 Dashboard 地址即可。
...
基于 Electron 构建，提供与 Web 端一致的管理体验：

- ✅ Windows / macOS / Linux 全平台支持
- ✅ 独立的登录会话与 Token 刷新，无需每次重新登录
- ✅ 实时监控面板、终端、运维工具一应俱全
- ✅ 自动更新（GitHub Releases）

下载地址：https://github.com/longdicode/ProberX/releases
